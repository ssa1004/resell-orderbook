 ...
 * Trade / outbox 이벤트가 다음 테스트의 {@code outbox.findAll()} / {@code findByStatus}
 * 결과에 섞여 들어와 단언이 꼬이는 일을 막는다.</p>
 *
 * <p>주의: {@code skus / products} 도 비우므로 자식 클래스의 {@code @BeforeEach} 가
 * 부모의 {@code @BeforeEach} 뒤에 실행되어 새 Sku 를 만들도록 JUnit 5 의 호출 순서에
 * 의존한다 (부모 → 자식). {@code shedlock / flyway_schema_history} 는 운영 메타라
 * 건드리지 않는다.</p>
 */
