...
/**
 * application.yml 의 {@code market.fee.*} 값으로 FeePolicy 제공. 단순/안전.
 *
 * <p>실시간 정책 변경이 필요하면 admin DB 기반 provider 로 교체 가능.</p>
 */
@Component
@RequiredArgsConstructor
public class YamlFeePolicyProvider implements FeePolicyProvider {

    private final FeeProperties props;

    @Override
    public FeePolicy current() {
        Currency currency = Currency.getInstance(props.currency());
