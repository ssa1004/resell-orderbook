...
/**
 * 매칭 엔진 — 도메인 서비스. *순수 함수* — 영속성/락/I/O 무관.
 *
 * <p>호출자 (PlaceListingService 등) 가 같은 SkuId 의 상대편 best 호가를 *원자적으로 읽고*
 * (DB FOR UPDATE 또는 분산락) 이 함수에 넘긴다. 매칭 조건 만족 시 Trade 가 반환되고, 호출자가
 * Listing/Bid 의 상태를 MATCHED 로 마킹한 후 모두 같은 트랜잭션에서 커밋한다.</p>
 *
 * <p>가격 결정 (taker/maker):</p>
 * <ul>
 *   <li>새 ASK 가 기존 BID 와 매칭 → 가격 = 기존 <strong>BID</strong> 의 가격 (BID maker)</li>
 *   <li>새 BID 가 기존 ASK 와 매칭 → 가격 = 기존 <strong>ASK</strong> 의 가격 (ASK maker)</li>
 * </ul>
 *
 * <p>거름 조건: 둘 다 {@link Listing#isMatchableAt(Instant)} / {@link Bid#isMatchableAt(Instant)} 통과해야 함
 * (status=ACTIVE + 만료 전). 상태만 ACTIVE 이고 만료된 호가는 race window 에서 매칭 안 되도록 차단.</p>
 */
public final class MatchEngine {
    ...
    /** BuyNow — 즉시 구매. 사용자가 BID 등록 없이 Lowest ASK 를 직접 매수. */
    ...
    /** SellNow — 즉시 판매. 사용자가 ASK 등록 없이 Highest BID 를 직접 매도. */
