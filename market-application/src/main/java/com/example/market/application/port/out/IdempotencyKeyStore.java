 ...
 *
 * <p><b>release 의 용도</b>: acquire 직후 비즈니스 로직이 실패해 트랜잭션이 rollback 되면,
 * 키 점유는 Redis 에 남아 같은 키로 *legitimate retry 가 막힘* (TTL 기간 동안 DoS).
 * {@link com.example.market.application.service.IdempotentExecution} 이 트랜잭션 hook
 * 으로 자동 release 처리 — 호출자는 직접 release 부르지 않음.</p>
 */
