 ...
 * ### begin 시 PK 충돌
 *
 * 두 thread 가 동시에 진입하면 begin 두 번째가 [CompensationLogStore.DuplicateBeginException]
 * 으로 떨어진다. 이 시점에 다시 [CompensationLogStore.find] → IN_PROGRESS 가 보이면
 * 다른 thread 가 처리 중이므로 [DuplicateInProgressException] 으로 호출자에게 신호.
 */
@Component
class CompensationGuard(
    private val store: CompensationLogStore,
     ...
     */
    class DuplicateInProgressException(op: String, key: String) :
        RuntimeException("compensation in progress by another worker: op=$op key=$key")

    /**
     * idempotent 하게 보상 1건을 실행.
     *
     * action 의 반환값은 [Outcome] — 성공/실패 + 외부 식별자 + 코드/메시지. 이 정보가
     * compensation_log row 에 박혀 다음 재시도 때 캐시로 재사용됨.
     *
     ...
     * @param action      실제 외부 호출. 입력은 캐시된 결과 ([Outcome], `null` = 신규).
     */
    fun <T> runOnce(
        operation: String,
        businessKey: String,
        action: Function<Outcome<T>?, Outcome<T>>,
    ): Outcome<T> {
        ...
        if (existing.isPresent) {
            ...
            if (entry.isCompleted()) {
                log.info(
                    "compensation 캐시 hit op={} key={} externalId={}",
                    operation, businessKey, entry.externalId,
                )
                @Suppress("UNCHECKED_CAST")
                val cached = Outcome.completed<Any?>(
                    entry.externalId, entry.responseCode, entry.responseMessage, null,
                ) as Outcome<T>
                return cached
            }
            if (entry.isInProgress()) {
                throw DuplicateInProgressException(operation, businessKey)
            }
            ...
            log.warn(
                "compensation 이전 시도가 FAILED 로 남음 — 새 키로 재시도 권장 op={} key={}",
                operation, businessKey,
            )
            // 이 케이스는 호출자에게 다시 던지지 않고 그대로 캐시 결과 (FAILED 결과) 를 반환.
            // 호출자가 응답 코드를 보고 분기.
            @Suppress("UNCHECKED_CAST")
            val cached = Outcome.failed<Any?>(entry.responseCode, entry.responseMessage, null) as Outcome<T>
            return cached
        }
        ...
        } catch (e: CompensationLogStore.DuplicateBeginException) {
            // 다른 thread 가 동시에 begin 했다 — 다시 find 해서 분기.
            val raced = store.find(operation, businessKey)
        ...
        try {
            ...
            try {
                store.fail(
                    operation, businessKey,
                    "EXCEPTION", e.javaClass.simpleName, clock.instant(),
                )
            } catch (failEx: RuntimeException) {
                e.addSuppressed(failEx)
                log.warn(
                    "compensation_log fail 기록 실패 op={} key={} (원인 예외는 그대로 전파)",
     ...
     *                      외부 식별자 ([externalId]) 만 있으면 도메인 결과를 재구성할 수
     *                      있는 책임을 진다.
     */
    @JvmRecord
    data class Outcome<T>(
        ...
        companion object {
            @JvmStatic
            fun <T> completed(externalId: String?, code: String?, message: String?, result: T?): Outcome<T> =
                Outcome(true, code, message, externalId, result)

            @JvmStatic
            fun <T> failed(code: String?, message: String?, result: T?): Outcome<T> =
                Outcome(false, code, message, null, result)
        }
    }

    companion object {
        private val log = LoggerFactory.getLogger(CompensationGuard::class.java)
    }
}
