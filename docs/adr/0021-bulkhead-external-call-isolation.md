...
2. 호가창 / 검수 슬롯 같이 외부 호출과 무관한 endpoint 까지 같이 느려짐 — 한 외부 의존의
   장애가 시스템 전체로 번지는 형태 (cascade failure)
...
사용자 요청 timeout 이 먼저 만료되어 queue 가 이미 죽은 요청만 채워진다.
...
- **`@Bulkhead` annotation 직접 적용** — Resilience4j AOP 가 어댑터 메서드를 자동 wrap. 코드는
  더 짧지만 어떤 풀이 어떻게 격리되는지 코드에서 즉시 안 보인다. 명시 데코레이터가 의도
  명확.
