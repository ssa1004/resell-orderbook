...
또 한 가지 위험은 **cache stampede** — 단순한 cache-aside 만 적용하면 TTL 이 0 이 되는 순간
여러 thread 가 동시에 cache miss 를 보고 모두 DB 로 몰려간다 (영문 자료에서는 "thundering
herd" 라 부르는 동일 현상). 일반 트래픽 + 그 순간 N 개의 무거운 같은 쿼리가 한꺼번에 들어와
DB 가 ms 단위로 압박을 받게 된다.
