...
두 서비스 모두 외부 호출 (PG / 은행) 이 핵심. 외부 호출의 응답이 유실되는 시나리오에서는
...
결제 / 송금 도메인의 표준 패턴은 **명시 idempotency key + 결과 캐시** —
[Stripe Idempotent Requests](https://stripe.com/docs/api/idempotent_requests) 가 대표적인
공개 표준이다. 외부 호출 직전에 idempotency key 로 자리를 점유 (DB row INSERT) 하고,
외부 응답을 받아 같은 row 에 결과를 저장한다. 재시도 시 row 가 있으면 외부 호출 없이
캐시된 결과를 그대로 반환.
...
일어났는지의 단서가 메인 트랜잭션의 결과와 분리되어 추적 가능.

이 분리가 중요한 이유: action (외부 호출) 도중 예외 → 메인 트랜잭션 rollback. compensation_log
가 메인 트랜잭션 안에 있었다면 같이 rollback 되어 첫 호출이 일어났던 흔적이 사라진다. 다음
...
재구성하는 책임을 진다. 이는 작은 트레이드오프 — result 자체를 DB 에 JSON 으로 직렬화 보관하는
방법도 있으나 envelope 형식 (스키마 진화 / 호환성) 관리 부담이 더 크다고 보고 일단 외부
식별자만 캐싱.
...
- **PG idempotency key 만 외부에 넘기고 자체 로그 없이** — PG 가 idempotency 를 지원해도 응답이
...
- (단) Outcome.result 가 캐시 hit 시 null — 호출자가 externalId 로 재구성. 일부 도메인 정보가
  유실될 수 있어 외부 식별자만으로 일관 처리가 가능한 보상에만 적용
